...
import type { FlagConfig, SpriteConfig, EmojiConfig, FlagCdnExt, FlagApiExt } from "~/types";
...
const flagConfig = computed<FlagConfig>(() => refProps.flag.value || { strategy: DEFAULT_STRATEGY, size: DEFAULT_SIZE });
...
const styles = computed(() => {
    ...
    if (isEmojiStrategy.value) {
        ...
        return {
            fontSize: cfg.fontSize && (typeof cfg.fontSize == "string" ? cfg.fontSize : `${cfg.fontSize}px`),
        };
    }
...
const apiUrl = computed(() => {
    ...
    if (cfg.cdn == "flagapi") {
        ...
        return `https://${FLAG_API_URL}/${isoCode.value.toUpperCase()}/${cfg.style || DEFAULT_STYLE_FLAG_API}/${cfg.size || DEFAULT_SIZE}.png`;
    }

    if (cfg.cdn == "flagcdn") {
        cfg = flagConfig.value as FlagCdnExt;
        return `https://${FLAG_CDN_URL}/${cfg.size || DEFAULT_SIZE_FLAG_CDN}/${isoCode.value}.png`;
    }
});
</script>
